...
public class ProduitViewModel : ObservableObject{
    ...
    var url:String = "http://172.17.5.221:3000/api/product/"
 ...
 /*  init() {
    ...
    func GetProducts(completed: @escaping (Bool,[Product]?) -> Void){
        AF.request("http://172.17.9.202:3000/api/product", method: .get)
            ...
            .responseData{ response in
                ...
                case.success:
                    for singleJsonItem in JSON (response.data!){
                        self.ListProducts.append(self.makeItem(jsonItem: singleJsonItem.1))
                    }
                    completed(true,self.ListProducts)
